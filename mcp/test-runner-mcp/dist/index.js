...
function moduleResultsDir(module) {
    return join(MODULES_DIR, module, "__tests__", "results");
}
function discoverModuleTests(module) {
    const dir = moduleTestsDir(module);
    if (!existsSync(dir))
        return [];
    const results = [];
    for (const f of readdirSync(dir)) {
        if (!f.endsWith(".json") || f.includes("-result") || f === "TEST-DESIGN-SUMMARY.json")
            continue;
...
function getLastResult(testId) {
    ...
    for (const mod of KNOWN_MODULES) {
        ...
        if (!existsSync(dir))
            continue;
        const candidates = readdirSync(dir).filter((f) => f.startsWith(testId) && f.endsWith("-result.json"));
    ...
    if (existsSync(RESULTS_DIR)) {
        const candidates = readdirSync(RESULTS_DIR).filter((f) => f.startsWith(testId) && f.endsWith("-result.json"));
        if (candidates.length > 0) {
            try {
...
function runTests(files, options = {}) {
    return new Promise((resolve) => {
        const args = ["src/test-runner/index.ts"];
        if (options.cleanup)
            args.push("--cleanup");
...
// ─── MCP Server ───────────────────────────────────────────────────────────────
const server = new Server({ name: "test-runner-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
// ─── Tool: list_tests ─────────────────────────────────────────────────────────
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "list_tests",
                description: "List all available test files. Filter by module (e.g. 'matching', 'booking') or type ('module' | 'journey'). Returns testId, scenarioName, artifactStatus, and file path for each test.",
                inputSchema: {
                    type: "object",
                    properties: {
                        module: {
            ...
            {
                name: "run_tests",
                description: "Run one or more test files by path. Pass explicit file paths or use run_module for convenience. Returns combined stdout output from the runner including pass/fail per test.",
                inputSchema: {
                    type: "object",
                    required: ["files"],
                    properties: {
            ...
            {
                name: "run_module",
                description: "Run all tests for a specific module in parallel. Equivalent to passing all __tests__/*.json files for that module to the runner.",
                inputSchema: {
                    type: "object",
                    required: ["module"],
                    properties: {
            ...
            },
            {
                ...
                description: "Get the most recent result JSON for a specific test by testId (e.g. 'MOD-MATCHING-003'). Returns the full result report including status, stepResults, assertionSummary, and failures.",
                inputSchema: {
                    type: "object",
                    required: ["testId"],
                    properties: {
            ...
            },
            {
                ...
                description: "Check whether all required services (API server, database, Docker, Mailpit) are running. Returns status per service plus fix instructions for anything that is down.",
                inputSchema: {
                    type: "object",
                    properties: {},
                },
            },
            {
                ...
                description: "Get a pass/fail summary across all tests for a module, or all modules if none specified. Shows which tests have results, which are pending (no result file yet), and last run status.",
                inputSchema: {
                    type: "object",
                    properties: {
                        module: {
        ...
        for (const mod of modulesToScan) {
            ...
            for (const t of tests) {
                ...
                rows.push({
                    ...
                    testId: t.testId,
                    scenarioName: t.scenarioName,
                    lastStatus: result ? result.status : "NO_RESULT",
