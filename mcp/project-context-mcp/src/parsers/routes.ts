...
export interface EndpointContract {
  ...
  summary?: string;
  auth: {
    kind: "public" | "user" | "internal";
    roles?: Array<"CLIENT" | "COMPANION">;
    notes?: string[];
  };
  handler?: string;
  sourceFile: string;
...
function findLeadingCommentBlock(fullText: string, matchIndex: number): string | undefined {
...
function extractHandler(argsText: string): string | undefined {
  // Heuristic: last dotted identifier is usually the controller handler.
  const matches = Array.from(argsText.matchAll(/\b([A-Za-z_$][\w$]*\.[A-Za-z_$][\w$]*)\b/g)).map(
    (m) => m[1]
  );
...
export function parseExpressRouterRoutes(sourceFile: string, fileText: string): EndpointContract[] {
  ...
  for (const m of fileText.matchAll(re)) {
    ...
    const roles = Array.from(argsText.matchAll(/requireRole\(\s*["'](CLIENT|COMPANION)["']\s*\)/g)).map(
      (x) => x[1] as "CLIENT" | "COMPANION"
    );

    const notes: string[] = [];
    let kind: "public" | "user" | "internal" = "public";
    ...
    if (roles.length > 0) notes.push(`requireRole(${roles.join(",")})`);
