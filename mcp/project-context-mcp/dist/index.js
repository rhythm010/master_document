...
import { getCoreSdsPath, getPrismaSchemaPath, getRouteFileForModule, KNOWN_MODULES, listMasterDocFiles, resolveLatestFeatureSdsPath, } from "./indexer.js";
...
const server = new Server({ name: "project-context-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
function safeRead(path) {
    assertSafePath(REPO_ROOT, path);
    return cache.readText(path);
}
function parseInvariantsFromCoreSds(md) {
...
function buildModuleContract(module) {
    ...
    const authSummary = endpoints.reduce((acc, e) => {
        acc.total++;
        acc[e.auth.kind]++;
        return acc;
...
function buildFeatureSds(feature) {
    ...
    const sectionText = (titleIncludes) => {
    ...
    const nonGoals = Array.from(new Set([
...
function buildSchemaEntity(entity) {
...
function buildSupportMatrix() {
    ...
    const codeEndpoints = moduleContracts.flatMap((mc) => (mc.endpoints ?? []).map((e) => ({ module: mc.module, method: e.method, path: e.path })));
...
function buildBusinessRules(topic, limit = 50) {
    ...
    // master docs
    for (const f of listMasterDocFiles())
        sources.push({ path: f, kind: "master-doc" });
    const hits = [];
    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "Return structured business rules, state transitions, validations, APIs, and non-goals for a feature (latest/current Feature SDS only).",
                inputSchema: {
                    type: "object",
                    required: ["feature"],
                    properties: {
