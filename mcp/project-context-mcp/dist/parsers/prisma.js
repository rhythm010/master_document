...
export function parsePrismaModel(schema, entity) {
...
export function listPrismaModels(schema) {
    const names = Array.from(schema.matchAll(/\bmodel\s+([A-Za-z_][A-Za-z0-9_]*)\s*\{/g)).map((m) => m[1]);
