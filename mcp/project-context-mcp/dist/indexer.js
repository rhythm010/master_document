...
import { FEATURE_SDS_DIR, CORE_SDS_PATH, MASTER_DOC_DIR, PRISMA_SCHEMA_PATH, BACKEND_MODULES_DIR, } from "./paths.js";
...
export function getRouteFileForModule(module) {
...
export function listMasterDocFiles() {
    if (!existsSync(MASTER_DOC_DIR))
        return [];
    return readdirSync(MASTER_DOC_DIR)
        .filter((f) => f.endsWith(".md"))
        .map((f) => join(MASTER_DOC_DIR, f));
}
export function resolveLatestFeatureSdsPath(featureKey) {
    ...
    if (files.includes(alias1))
        return join(FEATURE_SDS_DIR, alias1);
    // 2) Explicit current alias: <feature>_current.md
    const alias2 = `${featureKey}_current.md`;
    if (files.includes(alias2))
        return join(FEATURE_SDS_DIR, alias2);
    ...
    const candidates = files
        ...
        if (!m)
            return null;
        return { f, v: [Number(m[1]), Number(m[2]), Number(m[3])] };
    })
        .filter((x) => Boolean(x));
    if (candidates.length === 0) {
...
export function getCoreSdsPath() {
