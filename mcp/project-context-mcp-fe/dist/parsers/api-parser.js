...
function endpointFromCall(node) {
...
function responseTypeFromCall(node) {
    const params = node.typeParameters;
    if (params?.type === "TSTypeParameterInstantiation" && params.params[0])
        return nodeText(params.params[0]);
    return "";
}
function requestTypeFromCall(node) {
...
function collectQueryKeys(ast, endpointOrTag) {
    const keys = new Set();
    traverse(ast, {
        ObjectProperty(path) {
            ...
            if (valueText.toLowerCase().includes(endpointOrTag.toLowerCase()))
                keys.add(valueText);
        },
        VariableDeclarator(path) {
            const id = path.node.id;
            if (id.type !== "Identifier" || !id.name.toLowerCase().includes("querykey"))
                return;
...
export function parseApiContract(appRoot, endpointOrTag) {
