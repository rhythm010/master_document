...
function typeMemberToProp(member) {
...
function extractPropsFromTypeNode(node) {
    ...
    return members.map(typeMemberToProp).filter((prop) => Boolean(prop));
}
function findPropsTypes(ast, componentName) {
    const propsByTypeName = new Map();
    traverse(ast, {
...
function collectParamDefaults(param, props) {
    ...
    for (const property of param.properties) {
        if (property.type !== "ObjectProperty")
            continue;
        const propName = memberName(property.key);
...
function findComponentParamDefaults(ast, componentName, props) {
    const maybeComponentFunction = (path) => {
        const parent = path.parentPath?.node;
        const declaredName = path.node.type === "FunctionDeclaration"
            ? path.node.id?.name
            : parent?.type === "VariableDeclarator" && parent.id.type === "Identifier"
                ? parent.id.name
                : null;
        if (declaredName && declaredName !== componentName)
            return;
        for (const param of path.node.params)
            collectParamDefaults(param, props);
    };
    traverse(ast, {
...
function rankComponentFile(path, componentName) {
    const base = basename(path, extname(path));
    if (base === componentName)
        return 0;
    if (base.toLowerCase() === componentName.toLowerCase())
        return 1;
    if (base.replace(/[-_]/g, "").toLowerCase() === componentName.toLowerCase())
        return 2;
    return 3;
}
export function parseComponent(appRoot, componentName) {
