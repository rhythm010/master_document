...
import type { BackendGapStatusFilter, DesignTokenCategory } from "./types.js";
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "get_fe_design_tokens") {
      return { content: [jsonText(parseDesignTokens(APP_ROOT, String(args.category ?? "colors") as DesignTokenCategory))] };
    }

    if (name === "get_fe_backend_gaps") {
      const status = args.status ? String(args.status) as BackendGapStatusFilter : undefined;
