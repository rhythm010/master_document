...
import type { BackendGap, BackendGapStatusFilter } from "../types.js";
...
function sectionText(block: string, heading: string): string {
...
function normalizeFilterStatus(gap: BackendGap): BackendGapStatusFilter | string {
  const status = gap.status.toLowerCase();
  if (status.includes("resolved")) return "resolved";
  if (status.includes("open") || status.includes("progress") || status.includes("deferred")) return "open";
  if (gap.workaround && gap.workaround.toLowerCase() !== "none - blocker") return "workaround";
  return status;
}

export function parseBackendGaps(repoRoot: string, status?: BackendGapStatusFilter): BackendGap[] {
